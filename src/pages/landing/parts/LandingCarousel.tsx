...
import carousel1 from "assets/landing-carousel/landing-carousel-1.mp4";
      ...
      return (
        <div className="w-full h-full flex items-center justify-center">
          <video
            ...
            style={{ maxHeight: "420px" }} // Slightly smaller to fit inside the container
          >
  ...
  const renderCurrentSlide = () => {
    ...
    return (
      <div className="flex flex-col">
        ...
        <div className="w-full text-center px-2 sm:px-4 py-4 sm:py-6 flex flex-col justify-center overflow-visible" 
             style={{ height: "230px" }}>
          {!slide.isLastSlide ? (
            <>
              <h2 className={`font-medium px-1 sm:px-2 overflow-visible break-words ${slide.showLogo ? 'text-2xl sm:text-4xl mb-16 font-bold italic' : 'text-lg sm:text-xl'}`}>
                {slide.showLogo ? (
                  <span className="inline-flex items-center justify-center">
                    How <img src={company_logo} className="h-8 sm:h-10 mx-2 mb-2" alt="Company logo" /> Works?
                  </span>
                ) : (
                  slide.header
                )}
                  ...
                  {slide.underlinedText === "Perfect Match" ? (
                    <>
                      ...
                      <span className="inline-flex items-center align-middle">
                        <img src={sparkle_icon} className="w-5 h-5 mr-1" alt="" />{" "}
              ...
              {/* Button Row - Improved responsive layout */}
              {slide.buttons && (
                <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mt-3 sm:mt-4">
                  <button className="border border-orange-500 text-orange-500 px-3 sm:px-6 py-2 rounded-md hover:bg-orange-500 hover:text-white transition-colors text-sm sm:text-base">
                    {slide.buttons.left}
                  </button>
                  {slide.buttons.right && (
                    <button className="border border-orange-500 text-orange-500 px-3 sm:px-6 py-2 rounded-md hover:bg-orange-500 hover:text-white transition-colors text-sm sm:text-base">
